...
{
    public class SliderController : BackendBaseController
    {
        // GET: Backend/Slider
        ...
        public async Task<JsonResult> List(DataTableRequest request)
        {
            var _news = from b in db.Galleries
                        where b.IsDataActive && b.Category == "Slider"
                        select new
                        {
                            b.ID,
                            b.Title,
            ...
            foreach (var _rowData in _data)
            {
                ...
                _row.Add("<a href='/backend/slider/edit/" + _rowData.ID + "' title='Edit' class='mr5'><i class='icon-pencil7'></i></a> <a class='confirm mr5' data-confirm='Are you sure you want to delete this slider?' href='/backend/slider/delete/" + _rowData.ID + "' title='Delete'><i class='icon-trash'></i></a>");
                _rows.Add(_row);
            }
        ...
        public async Task<ActionResult> Create(Gallery model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    model.Category = "Slider";

                    db.Galleries.Add(model);
                    await db.SaveChangesAsync();

                    TempData["Notification"] = new Core.Entities.Common.Notification("Success", "Slider has been created successfully.");
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (_video != null)
                    {
                        ...
                        TempData["Notification"] = new Core.Entities.Common.Notification("Success", "Requested slider has been saved successfully.");
                        return RedirectToAction("Index");
                    }
                    else
                    {
        ...
        public async Task<ActionResult> Delete(int ID)
        {
            try
            {
                ...
                if (_video != null)
                {
                    ...
                    TempData["Notification"] = new Core.Entities.Common.Notification("Success", "Requested slider has been deleted successfully.");
                    await db.SaveChangesAsync();
                }
                else
                {
